...
mod tests {
    ...
    fn arclain_rule_adapter_keeps_supported_fields_and_existing_defaults() {
        ...
        assert_eq!(converted.actions.root_folder.as_deref(), Some("Library"));
        assert_eq!(converted.actions.output_name, None);
        assert_eq!(converted.actions.move_files.len(), 1);
        assert_eq!(converted.actions.move_files[0].pattern, "*.jpg");
        assert_eq!(converted.actions.move_files[0].target, "Images");
        assert!(!converted.actions.use_standard_layout);
    }
}
