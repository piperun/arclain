...
pub struct EnabledPluginSnapshot {
    plugins: Vec<(String, Arc<Mutex<PluginInstance>>)>,
}

impl EnabledPluginSnapshot {
    pub(super) fn new(plugins: Vec<(String, Arc<Mutex<PluginInstance>>)>) -> Self {
        Self { plugins }
    }
    ...
    pub fn plugin_instances(
        ...
        self.plugins
            .iter()
            .map(|(id, instance)| (id.as_str(), instance.clone()))
    }
