...
fn test_secrets_db_creation_and_encryption() {
    ...
    let db = SecretsDb::open(&secrets_path, &key.as_bytes()).expect("Failed to open secrets DB");
...
fn test_file_permissions_unix() {
    ...
    assert_eq!(
        mode & 0o777,
        0o600,
        "Secrets DB should have 600 permissions"
    );
    ...
    assert_eq!(
        dir_mode & 0o777,
        0o700,
        "Secrets dir should have 700 permissions"
    );
}
...
fn test_database_integration() {
    ...
    let personal_rule = fetched
        .iter()
        .find(|r| r.pattern.contains("personal"))
        .unwrap();
