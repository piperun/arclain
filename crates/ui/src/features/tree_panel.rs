...
fn build_tree_structure(folders: &[String]) -> Vec<TreeNode> {
    ...
    for folder in folders {
        if let Some(pos) = folder.rfind('/') {
            ...
            folder_map.entry(parent.to_string())
                .or_insert_with(Vec::new)
                .push(folder.clone());
        } else {
            // Top-level folder
            folder_map.entry(String::new())
                .or_insert_with(Vec::new)
                .push(folder.clone());
        }
    }
    ...
    fn build_node(path: &str, folder_map: &HashMap<String, Vec<String>>, indent: usize) -> TreeNode {
        ...
        if let Some(children) = folder_map.get(path) {
            for child in children {
                node.children.push(build_node(child, folder_map, indent + 1));
            }
        }
...
pub fn render(
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            if tree_item(ui, theme, "📦", archive_name, 0, is_root_selected, false).clicked {
...
fn render_tree_node(
    ...
    let is_expanded = state.expanded_folders.get(&node.full_path).copied().unwrap_or(false);
    ...
    let response = tree_item(ui, theme, icon, &node.name, node.indent_level, is_selected, has_children);
    ...
    if response.toggle_clicked && has_children {
        state.expanded_folders.insert(node.full_path.clone(), !is_expanded);
    }
...
fn tree_item(
    ...
    if ui.is_rect_visible(rect) {
        ...
        if has_children && indent_level > 0 {
            ...
            let toggle_response = ui.interact(toggle_rect, ui.id().with("toggle"), egui::Sense::click());
            if toggle_response.clicked() {
                toggle_clicked = true;
            }
            
            // Draw triangle (▶ or ▼)
            let triangle_icon = if icon.starts_with("📂") { "▼" } else { "▶" };
            ui.painter().text(
                toggle_rect.center(),
