...
pub fn render_preferences_dialog(
    ...
    show_dimmed_modal(
        ...
        |ui| {
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                let passwords_btn =
                    egui::Button::new("🔐 Manage Passwords…").min_size(egui::vec2(160.0, 32.0));
                ...
                if ui.add(rekey_btn).clicked() {
                    if let Some(file) = rfd::FileDialog::new().pick_file() {
                        rekey_path = Some(file.to_string_lossy().to_string());
                    }
                }
                ...
                if ui.add(move_btn).clicked() {
                    if let Some(path) = rfd::FileDialog::new()
                        ...
                        .save_file()
                    {
                        move_path = Some(path.to_string_lossy().to_string());
                    }
                }
            });
        },
    ...
    if save_clicked {
        let key_opt = if dialog.key_file_path.trim().is_empty() {
            None
        } else {
            Some(dialog.key_file_path.trim().to_string())
        };
        let db_opt = if dialog.secrets_db_path.trim().is_empty() {
            None
        } else {
            Some(dialog.secrets_db_path.trim().to_string())
        };
        let policy_opt = Some(dialog.encrypted_crc_policy.as_str().to_string());
        result = Some(PreferencesDialogResult::Save {
    ...
    if let Some(p) = rekey_path {
        result = Some(PreferencesDialogResult::RekeyVault {
            new_key_file_path: p,
        });
    }
    if let Some(p) = move_path {
        result = Some(PreferencesDialogResult::MoveVault { dest_path: p });
    }
