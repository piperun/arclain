...
pub fn render_password_rules_dialog(
    ...
    egui::Area::new(egui::Id::new("pass_rules_overlay_dim"))
        ...
        .show(ctx, |ui| {
            ...
            // Don't allocate input on the overlay to avoid intercepting scroll events.
        });
    ...
    egui::Area::new(egui::Id::new("pass_rules_modal"))
        ...
        .show(ctx, |ui| {
            ...
            child.vertical(|ui| {
                ...
                egui::ScrollArea::vertical()
                    .max_height(content.height() - 420.0)
                    .show(ui, |ui| {
                ...
                // Form in a nice table-like layout
                egui::Grid::new("password_rule_form")
                    ...
                    .show(ui, |ui| {
                        ...
                        // Pattern field with test button
                        ui.label(
                            egui::RichText::new("Pattern:")
                                .size(12.0)
                                .color(theme.colors.text_secondary)
    ...
    if dialog.show_regex_tester {
        render_regex_tester_modal(ctx, theme, dialog);
    }

    result
}

fn render_regex_tester_modal(
    ctx: &egui::Context,
    theme: &AppTheme,
    dialog: &mut PasswordRulesDialog,
) {
    // Dim overlay for the tester modal
    ...
    egui::Area::new(egui::Id::new("regex_tester_overlay"))
        ...
        .show(ctx, |ui| {
            let screen = ctx.screen_rect();
            ui.painter().rect_filled(screen, 0.0, egui::Color32::from_black_alpha(200));
        });
                    ...
                    if ui.button("📁 Pick Folder").clicked() {
                        if let Some(folder) = rfd::FileDialog::new().pick_folder() {
                            ...
                            if let Ok(entries) = std::fs::read_dir(&folder) {
                                ...
                                let pattern_str = dialog.regex_test_pattern
                                    .replace(".", "\\.")
                                    .replace("*", ".*")
                                    .replace("?", ".");
                                    ...
                                    for entry in entries.flatten() {
                                        if let Ok(file_type) = entry.file_type() {
                                            if file_type.is_file() {
                                                ...
                                                dialog.regex_test_results.push(RegexTestResult {
