...
`0.3.0`, declared by the sole source file at
...
`0.3.0` must contain a component implementing the `0.3.0` Wirt world, and the
host itself must be built for `0.3.0`. Any mismatch is rejected before plugin
initialization.

In particular:

- `0.3.1` is not assumed compatible with `0.3.0`.
- `0.4.0` is not assumed compatible with `0.3.0`.
